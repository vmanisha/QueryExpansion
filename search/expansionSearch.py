...
'''argv[1] = Session file argv[2] = index folder argv[3] = vector file / cat query folder / wikiIndex / (outfolder) argv[4] = category phrase folder / topic folder / queryIndex argv[5] = category cluster folder argv[6] = category Co-Occurrence file / term vector file argv[7] = dexter tagged file argv[8] = relevance judgements argv[9] = outFolder
...
'''def getWalkExpansion(query): def getTaskExpansion(query):
'''
...
'''lastQueryIndex = len(session) - 2
                ...
                if lastQueryIndex > -1:
                        ...
                        for entry in viewDocs[lastQueryIndex][:5]:
                                #print session[lastQueryIndex], 'docs',entry,
                                len(docText)
                                docText += getDocumentText(entry,
                                viewedFileFolder).lower()+' '

                        #print clickDocs[lastQueryIndex]
                        clickText = ''
                        for entry in clickDocs.values():
                                #print session[lastQueryIndex], 'click',entry,
                                len(docText)
                                for doc in entry:
                                        clickText +=
                                        getDocumentText(doc,viewedFileFolder).lower()+'
                                        '

                        #print
                        randTerms =
                        randWalk.expandLastWithSession(session,docText,
                        clickText,50 );
                        #docList =
                        searcher.getTopDocumentsWithExpansion(query,entExpTerms,1100,'content','id')
                        for qtype, terms in randTerms.iteritems():
                                if qtype not in randFile:
                                        randFile[qtype] =
                                        open(argv[6]+str(qtype)+'.RL2','w')
                                #k=1
                                docList =
                                searcher.getTopDocumentsWithExpansion(session[-1],terms,1100,'content','id')
                                k = 1
                                for dtuple  in docList:
                                        randFile[qtype].write(str(i)+' Q0
                                        '+dtuple[0]+' '+str(k)+'
                                        '+str(round(dtuple[1],2))+' rand\n')
                                        k +=1
