...
def mergeCategories(categoryNetwork,catQueryDist,weightMatrix):
	...
	for i in range(15):
		for entry in categoryNetwork.nodes():
			toDel = False
			if entry in catQueryDist  and len(catQueryDist[entry]) < 140 and entry not in avoid:
...
def returnFilteredNetwork(queryFile, skosFile,featMan, weightMatrix):
	...
	catNetwork , catQueryDist= mergeCategories(catNetwork,catQueryDist, weightMatrix)
