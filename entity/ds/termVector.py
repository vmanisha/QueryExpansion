...
class TermVector:
	...
	def __init__(self,fileName = None):
		...
		'''
		if fileName:
			self.vIndex, self.vsearcher = loadIndex(fileName,fileName[fileName.rfind('/')+1:])
	...
	def indexVector(self,indexLoc,indName):
		'''Indexes the vectors'''
		ischema = Schema(term= TEXT(stored=True,phrase=False),vector = TEXT(stored=True, phrase=False))
	...
	def indexSimilarity(self,indexLoc, indName, simFile):
		...
		ischema = Schema(word= TEXT(stored=True,phrase=False),score = TEXT(stored=True, phrase=False))
		if not os.path.exists(indexLoc):
			os.mkdir(indexLoc)
		...
		for line in open(simFile,'r'):
			...
			if split[0] != split[1]:
				try:
					word =  unicode((split[1]+'_'+split[2]).decode('unicode_escape').encode('ascii','ignore'))
