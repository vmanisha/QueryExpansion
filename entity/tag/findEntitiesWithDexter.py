...
def getEntityLinkWithDexter(spotFile, filterList=None):
  ...
  outFile = open(spotFile[spotFile.rfind('/') + 1:spotFile.rfind('.')] +
                 '_ent_info.txt', 'w')
  i = 1
  entityList = {}
  ...
  for entry in sorted(entityList.items(),
                      reverse=True,
...
'''
        ...
        args[3] = index of query on each line (esp when format is ID QUERY TIME
        CLICK etc
