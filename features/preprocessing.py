...
def findCatIndexes(catObj, featMan, coSessOcMan, coQueryOcMan):
  ...
  for clusPair in clustList:
    ...
    weightMatrix = findWeightMatrix(featMan, coSessOcMan, coQueryOcMan,
                                    phraseDict.keys())
  ...
  if maxDiam > 0:
    ...
    for i in range(len(clustList)):
      ...
      for j in range(i + 1, len(clustList)):
        ...
        weightMatrix = findWeightMatrix(featMan, coSessOcMan, coQueryOcMan, p1,
                                        p2)
  ...
  for ifile in os.listdir(argv[1]):
    try:
      ...
      if k > 0:
        ...
        writeDict(labelResult,
                  out + '/' + ifile[:ifile.rfind('.')] + '_' + str(k) + '.txt')
