...
def writeCatFiles(outFolder, wordFeat, clist):
  ...
  for cat, wordList in catWords.items():
    cat = cat.replace('/', '_')
    if len(wordList) > 10:
      oFile = open(outFolder + '/' + cat + '_' + str(len(wordList)) + '.txt',
                   'w')
  ...
  for cat, wordList in catWords.items():
    if len(wordList) > 20 and len(wordList) < 1500:
      ...
      for word in wordList:
        ...
        #cluster
      oFile = open(outFolder + '/' + cat + '_' + str(len(toCluster)) + '.txt',
                   'w')
    ...
    for k in range(wlen - 1):
      ...
      for j in range(k + 1, wlen):
        ...
        if key not in similarity:
          ...
          if len(toWrite) > 0:
            oFile.write(
                key + '\t' + '\t'.join('{0}:{1}'.format(x, y)
                                       for x, y in toWrite.items()) + '\n')
  return similarity
...
def writeFeaturesFromDict(wordFeat, outFile):
  ...
  for entry, featDict in wordFeat.items():
    oFile.write(entry + '\t' + '\t'.join('{0}:{1}'.format(x, y)
                                         for x, y in featDict.items()) + '\n')
  oFile.close()


'''[1] = query freq [2] = dexter tagged [3] = url feat [4] = output file
'''
