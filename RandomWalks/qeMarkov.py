...
import numpy
import math, re
from findQueryEntities import * 
from getSessionStats import *
import nltk
...
def updateNetwork(query, network,qp,searcher,tlc,field, ntype):
	...
	for rTuple in sorted(rList,reverse = True , key = lambda x : x [1]):
		if (len(terms) < 3000  and finder.ngram_fd[rTuple[0]] > 2) or (finder.ngram_fd[rTuple[0]] > 1.0 and rTuple[0][0] in query or rTuple[0][1] in query and len(terms) < 4000):
		#if (len(terms) < 3000  and finder.ngram_fd[rTuple[0]] > 2) or (rTuple[0][0] in query or rTuple[0][1] in query and len(terms) < 4000):
			a = rTuple[0][0]
			if len(a) > 2 and queryAlphabetFilter(a) and a not in stopSet and not queryWebsiteFilter(a):
				if a not in network:
					network[a] = {}
					terms.add(a)
				b = rTuple[0][1]
				if len(b) > 2 and queryAlphabetFilter(b) and b not in stopSet and not queryWebsiteFilter(b):
					if b not in network[a]:
						network[a][b]  = {}
						terms.add(b)
					network[a][b][ntype] = network[a][b].setdefault(ntype,0.0) + ((rTuple[1]-tmin)/(tmax-tmin))
	...
	for e1 , eDict in network2.iteritems():
		...
		for e2, tDict in eDict.iteritems():
			...
			if ntype in tDict:
				...
				network[e1][e2] = network[e1].setdefault(e2,0.0)+(factor*tDict[ntype])
				network[e2][e1] = network[e2].setdefault(e1,0.0)+(factor*tDict[ntype])
...
def findMarkovStats(argv):
	...
	wIndex, wsearcher, wtlc = loadIndex(wikiIndexDir,wikiIndexDir)
	qIndex, qsearcher, qtlc = loadIndex(queryIndexDir,queryIndexDir)

	
        qqp = QueryParser("session", schema=qIndex.schema, group=OrGroup)
        wqp = QueryParser("content", schema=wIndex.schema, group=OrGroup)
	...
	for session in getSessionWithNL(iFile):
		...
		if aTerms:
			...
			#queryNetwork = {}
			#wikiNetwork = {}
			terms=updateNetwork(query,totalNetwork,wqp,wsearcher,wtlc,'content','wiki')
			terms2=updateNetwork(query,totalNetwork,qqp,qsearcher,qtlc,'session','query')	
