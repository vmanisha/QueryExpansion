...
class HtmlFeatures:
	...
	def __init__(self, content):
		...
		self.toKeepTags = set(['h1','h2','h3','h4','h5','h6','table','div','p','b','i','a','img','li','input','strong'])
	...
	def tagDistribution(self):
		...
		tagDistStr = ','.join([str(round(x[1],3)) for x in sorted(tagDist.items())])
		return tagDistStr
	...
	def outlinksWithDiffDomain(self,url):
		...
		outLinkStr = ','.join([str(round(x[1],3)) for x in sorted(outlinkDist.items())])
		return outLinkStr
	...
	def outlinksToTextRatio(self):
		...
		try:
			...
			pass
		retString = ','.join([str(round(x[1],3)) for x in sorted(outlinkFeat.items())])
		return retString
	...
	def summaryTagSpan(self,queryTerms,qLen):
		...
		try:
			...
			print ex
			pass				
		retString1 = ','.join([str(round(minTag[y],3)) for y in sorted(minTag.keys())])
		retString2 = ','.join([str(round(spanFeat[y],3)) for y in sorted(spanFeat.keys())])
	...
	def getTextFeature(self, qTerms,url):
		...
		retString = ','.join([str(round(metrics[y],3)) for y in sorted(metrics.keys())])
		return retString				
	...
	for line in open(argv[2],'r'):
		...
		if pid > k:
			...
			#fetch the content
				...
				string = ','.join(str(round(val[1],3)) for val in sorted(metrics.items()))
				string += ','
				string += ','.join(str(round(val[1],3)) for val in sorted(queryDocMetrics.items()))
				string += ','
				string += ','.join(str(round(val[1],3)) for val in sorted(textMetrics.items()))
