        ...
        for entry in catList[i]:
          for entry2 in catList[j]:
            ...
            if entry not in catCoFreq:
              catCoFreq[entry] = {}
            catCoFreq[entry][entry2] = 1.0 if entry2 not in catCoFreq[
                entry
            ] else catCoFreq[entry][entry2] + 1
            catCoFreq[entry2][entry] = 1.0 if entry not in catCoFreq[
                entry2
            ] else catCoFreq[entry2][entry] + 1
        j += 1
        if j == len(catList):
          i += 1
          j = i + 1
...
def main(argv):
  ...
  for entry, elist in catCoFreq.iteritems():
    print entry, '\t', '\t'.join('{0}:{1}'.format(x[0], x[1])
                                 for x in sorted(elist.iteritems(),
                                                 reverse=True,
                                                 key=lambda x: x[1]))
