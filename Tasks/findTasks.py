...
def getSessionFeatures(session,sId, user):
	...
	for i,j,score, cosine, jaccard, edit, dist, timeDiff, urlMatch in getPairFeatures(session):
...
def getHTC(jScore,wxScore,tscore,lsession):
	...
	while i < len(cluster)-1 :
		if len(cluster[i]) > 0 and len(cluster[j]) > 0:
			...
			t1 = max(cluster[i])
			t2 = max(cluster[j])
			sim = min(wxScore[h1][h2],wxScore[h1][t2],wxScore[t1][h2],wxScore[t1][t2])
			if sim > tscore:
				#merge the clusters
...
def formatResults(session, jsonObject, label, cTask, sc1):
	...
	for j in range(len(cTask)):
		...
		taskList[i][uquery] = 1 if uquery  not in taskList[i] else taskList[i][uquery] + 1
...
def main(argv):
	...
	print args
	tscore = {QCC: float(args.qccThresh),HTC:float(args.htcThresh),DBS:float(args.dbScanThresh)}
