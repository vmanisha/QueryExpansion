...
import math
import ast
from Utils import SYMB, stopSet, queryAlphabetFilter
#import nltk
...
def getProtEntityPhrases(fileName,outDir):
				...
				if len(cats) > 2:
					...
					for cat in categories:
						...
						if rho > 0.01:
							...
							for qRep in qsplit:
								qRep = qRep.strip()
								if len(qRep) > 2 and qRep not in stopSet and queryAlphabetFilter(qRep):
	...
	cqn = 20 #no of queries/phrase per category
	qn = 10	 #no of times query/phrase occurs in cat
	uqn = 15 #no of unique queries/phrase with occurance > qn
	mn = 10  #no of times this phrase match occured
	catPhraseTotal = {}
	catEntityTotal = {}
